...
infoTable <- data.frame(samples, imgs, spotfiles, json, 
                        ...
                        sample_id = c("FF1", "FFPE1", "FFPE2", "FFPE3", "FFPE4", "FFPE5", "FFPE6", "FFPE7"), 
                        stringsAsFactors = F)
...
se2 <- readRDS("~/STUtility/saved/se_object")
se3 <- readRDS("~/10x/R_objects/se.list")[[1]]

conv <- setNames(c("FF6", "FF7"), nm = c("Hippo_A", "Hippo_B"))
se2$sample_id <- conv[se2$sample_id]

conv <- setNames(c("FF2", "FF3", "FF4", "FF5"), nm = paste0("sample_", 1:4))
se3$sample_id <- conv[se3$sample_id]

```

```{r, fig.width=10, fig.height=10}

gene_attr_melt <- do.call(rbind, lapply(list(se, se2, se3), function(se) {
