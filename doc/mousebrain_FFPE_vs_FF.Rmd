...
samples <- c("../data/mousebrain/V19T26-039_A1/filtered_feature_bc_matrix.h5",
             "../data/mousebrain/public_data_FF/V1_Adult_Mouse_Brain_filtered_feature_bc_matrix.h5")

imgs <- c("../data/mousebrain/V19T26-039_A1/spatial/tissue_hires_image.png",
          "../data/mousebrain/public_data_FF/spatial/tissue_hires_image.png")

spotfiles <- c("../data/mousebrain/V19T26-039_A1/spatial/tissue_positions_list.csv",
               "../data/mousebrain/public_data_FF/spatial/tissue_positions_list.csv")

json <- c("../data/mousebrain/V19T26-039_A1/spatial/scalefactors_json.json",
          "../data/mousebrain/public_data_FF/spatial/scalefactors_json.json")
...
```{r read_se.FFPE include=FALSE, eval=TRUE}
se.FFPE <- readRDS("../R_objects/se.FFPE")
se.FF <- readRDS("../R_objects/se.FF")
```
...
```{r read_se.FFPEinclude=FALSE, eval=TRUE}
de.markers.FFPE <- readRDS("../R_objects/de.markers.FFPE")
de.markers.FF <- readRDS("../R_objects/de.markers.FF")
```
...
ggsave(filename = "~/FFPE/analysis/figures/suppl_figure_8/props.pdf", width = 7, height = 4)
...
se <- SubsetSTData(se, spots = rownames(subset(se[[]], !labels %in% "remove")))
saveRDS(object = se@assays$SCT@data, file = "~/FFPE/app/data/data")
saveRDS(object = se@assays$stereoscope@data, file = "~/FFPE/app/data/props")
saveRDS(object = GetStaffli(se)@meta.data, file = "~/FFPE/app/data/coords")

se <- SwitchResolution(se, xdim = 1000, verbose = T)
saveRDS(object = GetStaffli(se)@dims, file = "~/FFPE/app/data/imdims.list")

st.object <- GetStaffli(se)
for (i in c("1", "2")) {
  im <- st.object@rasterlists$processed[[i]]
  dims <- st.object@dims[[i]]
  png(filename = paste0("~/FFPE/app/www/imgs/", i, ".png"), width = dim(im)[2], height = dim(im)[1], res = 300)
  par(mar=c(0,0,0,0))
  plot(im)
  dev.off()
}
...
saveRDS(se, file = "../R_objects/se")
saveRDS(se.FFPE, file = "../R_objects/se.FFPE")
saveRDS(de.markers, file = "../R_objects/de.markers")
saveRDS(intersecting.markers, file = "../R_objects/intersecting.markers")
